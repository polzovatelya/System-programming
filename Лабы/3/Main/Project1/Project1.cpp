...
{
    TCHAR str[50] = L"Дочернее окно ", n[50] = L"";
    int* k = (int*)lParam;
    _itow_s(++(*k), n, 10);
    wcscat_s(str, n);
    ...
    case WM_COMMAND:
        {
            ...
            case 1005:
                ...
                SendMessage(LAB2, WM_RBUTTONUP, 0, MAKELPARAM(mouse.x, mouse.y));
                //SendMessage(LAB2, WM_RBUTTONUP, 0, 0);
                break;
            
            case 1006: {
                EnumChildWindows(LAB2, num_child, (LPARAM)&cnt);
                EnumWindows(num, (LPARAM)&cnt);
            }
                break;
            case IDM_ABOUT:
